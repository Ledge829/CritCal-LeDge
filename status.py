...
def parse_ratio(ratio_string):
    ...
    while len(values) < 3:
        values.append(None)
    return {"24h": values[0], "7d": values[1], "30d": values[2]}


def derive_last_check(response_times, logs=None):
    """
    Prefer response_times (per-check granularity) when UptimeRobot actually
    returns data for it -- but in practice this array often comes back
    empty even when requested, so fall back to the newest entry in the
    logs array (state-change events), which is reliably populated.
    """
    if response_times:
        newest = max(response_times, key=lambda r: r.get("datetime", 0))
        if newest.get("datetime"):
            return newest.get("datetime")

    if logs:
        newest_log = max(logs, key=lambda l: l.get("datetime", 0))
        return newest_log.get("datetime")
...
def build_summary(monitors):
    ...
    return {
        ...
        "paused": paused, "unknown": unknown, "overall_health": health,
    }
...
def get_uptime_status(monitor_id=None):
    ...
    for monitor in data.get("monitors", []):
        ...
        last_check_unix = derive_last_check(response_times, monitor.get("logs", []))
        ...
        monitor_data = {
            ...
            "uptime": {**parse_ratio(monitor.get("custom_uptime_ratio")), "all_time": monitor.get("all_time_uptime_ratio")},
...
def status():
    ...
    try:
        ...
        return jsonify({"success": False, "status": "error", "error": "Timed out while contacting UptimeRobot."}), 504
...
def health():
    return jsonify({"success": True, "status": "healthy", "service": "CritCal"}), 200

...
from akasha_client import get_akasha_data, get_akasha_data_debug
...
@app.route("/debug/akasha", methods=["POST"])
def debug_akasha():
    """
    Temporary troubleshooting endpoint -- shows the REAL error from the
    Akasha integration instead of the silent None that /rate/uid returns
    on any failure. Remove once Akasha integration is confirmed working.
    """
    body = request.get_json(silent=True, force=True)
    if not body or "uid" not in body or "character" not in body:
        return jsonify({"error": "Need both 'uid' and 'character' fields."}), 400

    result = get_akasha_data_debug(body["uid"], body["character"])
